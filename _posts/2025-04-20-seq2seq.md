...
- 一个整数序列 $x = [x_1, x_2, \dots, x_n]$，其中：
  - 序列长度 $n \leq$ `max_length`。
  - 每个整数 $x_i$ 满足 $1 \leq x_i \leq$ `cfg.vocab_size - 2`。
 ...
 **训练**
- 模型进行 `evaluate_round` 轮独立训练，每轮训练进行 `steps` 次更新。
- 每次训练后，模型会保存并用于评估。

 **评估**
- 评估模型的性能时，随机生成 `test_samples` 个样本计算指标的平均值：
  1. **PSA**（位置序列准确率）
  2. **LCSR**（最长公共子序列比率）
...
针对第一个问题，我们当然可以很简单的使用one-hot encoding来最大程度的保留输入元素的细节，但是由于 `max_length` 可能会很大，这样会导致维度灾难。所以此处我选择的是embedding的方式，它可以将范围很大的int全部映射到固定长度的特征
